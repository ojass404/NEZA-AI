...
interface UploadZoneProps {
  onStartAnalysis: (file: File | { name: string; size: number; type: string; imageUrl?: string }) => void;
  isProcessing: boolean;
}

export const UploadZone: React.FC<UploadZoneProps> = ({ onStartAnalysis, isProcessing }) => {
  const [selectedFile, setSelectedFile] = useState<File | { name: string; size: number; type: string; imageUrl?: string } | null>(null);
  const [isDragging, setIsDragging] = useState(false);
  const fileInputRef = useRef<HTMLInputElement>(null);

  const sampleFiles = [
    { name: 'scan-coastal-alpha.png', size: 2.4 * 1024 * 1024, type: 'PNG sonar image', imageUrl: '/sonar-samples/scan-coastal-alpha.png' },
    { name: 'scan-shelf-bravo.png', size: 2.8 * 1024 * 1024, type: 'PNG sonar image', imageUrl: '/sonar-samples/scan-shelf-bravo.png' },
    { name: 'scan-deep-charlie.png', size: 2.2 * 1024 * 1024, type: 'PNG sonar image', imageUrl: '/sonar-samples/scan-deep-charlie.png' },
  ];
